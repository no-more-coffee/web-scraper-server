...
async def read_items(skip: int = 0, limit: int = 100, db_context: Session = Depends(get_db)):
...
async def read_item(request: Request):
    ...
    items = response.json()
    return templates.TemplateResponse("index.html", {"request": request, "page": 1, "items": items})
